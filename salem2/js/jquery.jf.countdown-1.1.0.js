...
(function($) {
    ...
    function ObjJfCountdown(params, callback) {
        ...
        function initialise() {
            ...
            dateOpen.setHours(options.hour, options.minutes, 0);
            ...
            if(timeUTCNowMs <= timeUTCOpenMs) {
                ...
                counter.days = Math.floor(counter.seconds / options.daySec);
                if(counter.days > 0) {counter.seconds -= counter.days * options.daySec;}

                counter.hours = Math.floor(counter.seconds / options.hourSec);
                if(counter.hours > 0) {counter.seconds -= counter.hours * options.hourSec;}

                counter.minutes = Math.floor(counter.seconds / options.minuteSec);
